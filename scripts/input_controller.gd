...
##  RMB             context order: move / attack / harvest / dock / set rally point
##  A + LMB         attack-move        S  stop
##  Ctrl+1..9       make group         1..9  recall group
##  Esc / RMB       cancel placement / sell / repair mode

enum Mode { NONE, PLACE, SELL, REPAIR, ATTACK_MOVE }
		...
		if k.ctrl_pressed or k.meta_pressed:
			...
			if not valid.is_empty():
				...
				if now - _last_group_t < 0.4 and _last_group == num:
					G.camera.focus_on(valid[0].position)  # double-tap centres camera
				_last_group_t = now
				_last_group = num
...
func _process(_delta: float) -> void:
	# left button released over the HUD: finish the box select anyway
...
func entity_at(screen_pos: Vector2) -> Entity:
	...
	var best_d := 22.0
	for e in G.entities:
		if not e.alive or e is Structure:
			continue
	...
	for e in G.entities:
		if e.alive and e is Structure and e.contains_point(gp, 0.1):
			return e
	return null
...
func _context_order(pos: Vector2) -> void:
	...
	if t and t.team != G.local_team:
		for u in units:
			u.cmd_attack(t)
		Fx.marker(t.position, Color(1.0, 0.25, 0.2, 0.9))
		return
	# harvester-specific orders
	...
	if t and t is Structure and t.def_id == "refinery" and t.team == G.local_team:
		for hv in harvesters:
			hv.cmd_dock(t)
		harvesters = []
	elif G.map.crystal_at(cell) > 0.0:
		for hv in harvesters:
